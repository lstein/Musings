...
sub create_damfile {
    ...
    print $outfh pack('a4QQQZ*',MAGIC,HEADER,0,0,$abspath); 
...
sub open_sam_or_bam {
    ...
    my $infh;
    if ($infile =~ /\.bam$/) {
	open $infh,"samtools view $infile | sort | "  or die "samtools view $infile: $!";
    } else {
	open $infh,"sort $infile                 |"  or die "sort $infile: $!";
    }
    return $infh;
}
